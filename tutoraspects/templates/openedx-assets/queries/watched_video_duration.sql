...
select
    ...
    if(
        video_duration = 0, 0, count(distinct segments.segment_start)
    ) as segment_count,
    if(
        video_duration = 0,
        0,
        count(
            distinct case
                when watched_count > 1 then segments.segment_start else 0
            end
        )
...
from course_data
full join
    segments on segments.course_key = course_data.course_key
